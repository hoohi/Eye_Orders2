package com.eyeorders.util.printer.sumni;
...
import com.eyeorders.util.printer.TableItem;
...
public class SunmiPrintHelper {
    ...
    public void sendRawData(byte[] data) {
        if (sunmiPrinterService == null) {
            if (listener != null) {
                listener.onPrinterDisconnecting();
            }
            return;
        }
        try {
    ...
    public void printExample(Context context) {
        ...
        try {
            ...
            Bitmap bitmap = BitmapFactory.decodeResource(context.getResources(), R.drawable.eye_orders);
            sunmiPrinterService.printBitmap(bitmap, null);
    ...
    public void printTableItems(List<TableItem> items) {
        if (sunmiPrinterService == null) {
            if (listener != null) {
                listener.onPrinterDisconnecting();
            }
        ...
        for (TableItem item : items) {
            sendRawData(item.getBoldData());
            setFontSize(item.getFontSize());
            printTable(
                    item.getText(),
                    item.getWidth(),
                    item.getAlign()
            ...
            if (item.getLineFeedCount() > 0) {
                lineWrap(item.getLineFeedCount());
            }
        }
